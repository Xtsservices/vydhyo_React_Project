...
const DiagnosisMedication = ({ formData, updateFormData }) => {
  ...
  const fetchInventory = async () => {
    try {
      ...
      setMedicineOptions(response.data.data?.map(med => ({
        value: med.medName,
        label: med.medName,
      })) || []);
      toast.success('Medicine inventory loaded successfully');
    } catch (error) {
  ...
  const fetchTests = async () => {
    ...
    try {
      ...
      setTestOptions(response.data.data?.map(test => ({
        value: test.testName,
        label: test.testName,
      })) || []);
      toast.success('Test list loaded successfully');
    } catch (error) {
  ...
  const removeTest = (testName) => {
    const updatedData = {
      ...localData,
      selectedTests: localData.selectedTests.filter(test => test.testName !== testName)
    };
    setLocalData(updatedData);
    updateFormData(updatedData);
  ...
  const validateMedication = (medication) => {
    ...
    if (medication.timings.length !== medication.frequency) {
      toast.error(`Please select exactly ${medication.frequency} timing${medication.frequency > 1 ? 's' : ''} to match the frequency`);
      return false;
    }
    return true;
  };
  ...
  const addMedication = () => {
    ...
    if (localData.medications.length > 0 && !validateMedication(lastMedication)) {
      return;
    }
  ...
  const updateMedication = (id, field, value) => {
    const updatedData = {
      ...localData,
      medications: localData.medications.map(med => 
    ...
    if (['frequency', 'timings'].includes(field)) {
      const medication = updatedData.medications.find(med => med.id === id);
      if (medication.frequency && medication.timings.length > medication.frequency) {
        updateMedication(id, 'timings', medication.timings.slice(0, medication.frequency));
      }
    }
  };
              ...
              <div className="medication-row">
                <div>
                  ...
                  <AutoComplete
                    ...
                    style={{ width: '100%' }}
                    onSelect={(value) => updateMedication(medication.id, 'medName', value)}
                    onChange={(value) => updateMedication(medication.id, 'medName', value)}
                ...
                <div>
                  ...
                  <InputNumber
                    ...
                    onChange={(value) => updateMedication(medication.id, 'quantity', value)}
                    style={{ width: '100%' }}
                    min={1}
                  />
                </div>
              ...
              <div className="medication-timing-row">
                <div>
                  ...
                  <Select
                    ...
                    onChange={(value) => updateMedication(medication.id, 'timings', value)}
                    style={{ width: '100%' }}
                    allowClear
